---
name: "iidp-codebase-scan"
description: "运行 IIDP 存量代码库扫描流程（Codegraph CLI 全量遍历 → Codebook 证据采集 → 文档生成 → SRS 转译）。用法：/iidp-codebase-scan --source /path/to/repo"
...
本 skill 被用户直接调用时，默认进入**正式执行模式**。正式执行模式必须产出可交付的逆向工程文档，而不是验证 harness、演示 fixture 或规则脚本的近似结果。

正式执行模式的职责边界固定如下：

- Bash 只负责调用 CodeGraph CLI、复制默认配置、读写证据文件、执行 JSON 校验和质量验收。
- CodeGraph CLI 只负责事实采集：文件、节点、kind、nodeCount、callers、callees、route 等原始证据。
- Python、Node 或临时脚本只允许做机械辅助：JSON 校验、字段统计、格式转换、批量执行 CodeGraph 命令；**禁止用 Python、Node 或临时脚本替代大模型生成 TypeCard、业务文档、SRS 或 UML 语义说明**。
- 大模型负责所有需要理解、归纳、命名、摘要、业务语义解释和技术无关转译的生成任务。

如果当前环境无法调用大模型生成 `container_summary`、`deep_dive` TypeCard 或第 8/9 步文档，**无法调用大模型时，必须停止**，并在 `quality.json.document_errors` 与最终回复中说明“缺少大模型生成能力”；不得改用规则脚本伪造正式产物，也不得把验证 harness 输出标记为文档生成完成。

允许存在两种产物模式，但必须显式标记：

```text
generation_mode = "llm_prompted"      # 正式执行模式，允许通过质量门禁
generation_mode = "rule_based_harness" # 仅验证采集链路，不允许标记文档完成
```

正式执行模式下，`quality.json.generation_mode` 必须为 `"llm_prompted"`。若使用规则、脚本或 fixture harness 生成 TypeCard/文档，必须设置为 `"rule_based_harness"`，并强制：

```text
entry_coverage[entry.id].document_generation_complete = false
quality.doc_completeness_score = 0
最终回复明确说明：这是验证产物，不是正式逆向文档
```
...
for each file in <codebook_root>proposals/iidp-codebase-scan-*.md:
...
if --init-only:
  校验 JSON 可读、transport.mode == "cli"、commands 包含 version/status/files/query/query_by_kind/query_by_kind_without_language/callers/callees
  校验 typecard-policy.json 可读，且包含 kind_actions、decorator_rules、batch_generation
...
读取 `codegraph-tools.json.symbol_enumeration`。完整枚举必须使用 `symbol_enumeration.kinds`，不得复用 `tools.codegraph_query.kinds`；后者只用于 TypeCard 候选节点筛选。`symbol_enumeration.kinds` 必须覆盖 Codegraph 0.9.9 的全部原生 kind，例如 `file/module/class/struct/interface/trait/protocol/function/method/property/field/variable/constant/enum/enum_member/type_alias/namespace/parameter/import/export/route/component`。
...
for each file in inventory.json.files:
  ...
  for each native_kind in symbol_enumeration.kinds:
    if file.language 存在且 file.language != "unknown":
      query_args = render(tool_policy.commands.query_by_kind,
        binary=<binary>,
        source=<source>,
      ...
      query_args = render(tool_policy.commands.query_by_kind_without_language,
        binary=<binary>,
        source=<source>,
        index-total-nodes=limit,
        native-kind=native_kind,
    ...
    query_result = exec(query_args)
    if query_result.exit_code != 0 或 query_result.stdout 不是合法 JSON:
      将命令、文件、native_kind 和中文错误写入 quality.json.query_failures
      将同一错误写入 quality.json.symbol_enumeration.kind_query_failures
      将当前文件加入 incomplete_files
      继续下一个 native_kind

    results = parse_json(query_result.stdout)
    kind_symbols = 只保留 result.node.filePath == 当前文件 且 result.node.kind == native_kind 的全部结果
    kind_symbols = 按 result.node.id 去重
    per_kind_counts[native_kind] = kind_symbols 数量
    file_symbols += kind_symbols

    if results 原始数量 >= limit:
      将命令、文件、native_kind 和中文警告写入 quality.json.query_truncation_warnings
      将当前文件加入 incomplete_files

  symbols = file_symbols 按 result.node.id 去重后的全部结果
  ...
  将 per_kind_counts 写入 quality.json.symbol_enumeration.per_file_kind_counts[entry.id][file.path]
```

`"path:<精确文件路径>"` 是 CLI 查询条件，不等同于精确结果；必须再次执行 `node.filePath == 当前文件 && node.kind == 当前 kind` 的精确过滤。去重键固定为 `node.id`，不得使用名称、行号或 kind 代替。
...
精确过滤并按 `node.id` 去重后的全部节点均写入该入口的 `nodes.json`；第 6.5 步基于 `codegraph_query.kinds` 与 `typecard-policy.json` 为节点生成 TypeCard 计划，决定哪些节点直接生成元数据卡片、哪些节点需要深挖、哪些节点仅作为证据保留。`file/import/export/parameter/namespace` 等节点仍保留为完整性证据，不得从枚举计数中删除。

若任一文件的任一 kind 查询失败、返回非法 JSON、达到 `limit` 或数量无法对账，设置 `symbol_enumeration_complete = false`，该入口禁止进入第 8 步。不得回退为单次 all-kind `path:<精确文件路径>` 查询，避免重新触发 Codegraph 搜索爆栈；必须在 `nodes.json` 和 `quality.json.query_truncation_warnings` 中记录中文警告，且只有数量对账完全一致时才可判定该文件枚举完成。

#### 6.5 TypeCard 计划生成

读取 `<codebook_root>typecard-policy.json`、`semantic-kinds.json` 和当前入口的 `nodes.json`，生成：

```text
<codebook_root>evidence/<entry-id>/typecard-plan.json
```

TypeCard 计划器只解释已采集事实，不替代 Codegraph 索引。注解和装饰器来源优先级固定为：

1. `node.decorators`
2. `decorates` 关系指向的注解/装饰器节点
3. 当前节点源码片段兜底解析

兜底解析只能读取当前节点 `filePath/startLine/endLine` 对应源码片段，不得全库搜索注解，不得用注解关键字扩大入口范围。注解归一化规则来自 `typecard-policy.json.normalization`：移除 `@`、常见包名前缀和参数列表，例如 `@org.springframework.stereotype.Service("x")`、`@Service` 与 `Service` 统一归一为 `Service`。

计划动作固定为：

```text
metadata_only       直接生成确定性 TypeCard，不读取 callers/callees，不调用大模型
container_summary   基于容器节点自身、注解、签名、子节点摘要生成轻量 TypeCard
deep_dive           读取源码片段并查询 callers/callees，再进入批量 TypeCard 生成
evidence_only       仅保留在 nodes.json/typecard-plan.json 中，不生成独立 TypeCard
skipped             因策略或证据缺失跳过，并记录中文原因
```

生成规则：

```
for each node in nodes.json.nodes:
  native_kind = node.kind 或 node.native_kind
  decorators = 按 annotation_sources 读取并归一化
  semantic_kind = 先按 decorator_rules 匹配，其次按 semantic-kinds.json 的 native_kind/path/name/decorator 规则匹配，最后使用 fallback_semantic_kind
  action = 先按 decorator_rules.action 匹配，其次按 kind_actions 匹配，最后为 evidence_only

  if action == container_summary 且 decorator_rules.child_method_action 指向 deep_dive:
    找出该容器 contains 的 method/function 子节点
    排除 method_selection.skip_name_regex 命中的 getter/setter/toString/equals/hashCode 等低价值方法
    若 deep_dive_public_only == true，仅保留 visibility 为空或 public 的业务方法
    route 注解方法始终标记为 deep_dive

  写入 typecard-plan.json.planned_nodes[]
```

`typecard-plan.json` 中每个计划节点必须包含：
...
```json
{
  ...
  "file": "<node.filePath>",
  "native_kind": "method",
  "semantic_kind": "service",
  "typecard_action": "deep_dive",
  "action_reason": "Service 容器的 public 业务方法",
  "decorators": ["Service"],
  "parent_container": "<class-node-id-or-null>",
...
入口级统计必须写入 `quality.json.typecard_planning[entry.id]`，至少包含 `metadata_only`、`container_summary`、`deep_dive`、`evidence_only`、`skipped` 的数量。未知注解只记录为证据，按 `unknown_annotation_policy` 处理，不得自动猜测业务语义。

### 7. 节点深挖与 TypeCard 生成

第 7 步必须读取第 6.5 步生成的 `typecard-plan.json`，按 `typecard_action` 分层执行。不得绕过计划重新对全部节点执行 callers/callees。

```
metadata_only:
  使用 node 元数据、signature、decorators、parent_container、source_nodes 直接生成 TypeCard
  callers/callees 必须为空数组，并记录 relation_skipped_reason = "metadata_only 节点不查询调用关系"

container_summary:
  汇总容器节点自身、decorators、signature、docstring、子节点摘要和 source_nodes
  不查询 callers/callees
  必须使用 temp_typecard.json 进行大模型批量轻量生成

deep_dive:
  source_path = resolve(<source>, node.filePath)
  校验 source_path 仍位于 <source> 内
  source_code = 按 node.startLine 到 node.endLine 读取源码
  callers_args = render(tool_policy.commands.callers, binary, source, limit, node.name)
  callees_args = render(tool_policy.commands.callees, binary, source, limit, node.name)
  callers/callees = exec_json(...)
  必须使用 temp_typecard.json 进行大模型批量生成

evidence_only/skipped:
  不生成独立 TypeCard
```
...
只有 `deep_dive` 节点允许执行 `callers`/`callees`。任一需要读取源码的计划节点发生源码路径越界、源码读取失败，或 `deep_dive` 节点的 `callers`/`callees` 命令失败、返回非法 JSON 时，将错误写入 `quality.json.query_failures`，按 `transport.error_policy` 阻断当前入口进入文档生成。`metadata_only`、`container_summary`、`evidence_only` 节点不得因为未查询调用关系而报错。

将节点、计划动作、调用者、被调用者和语义 kind 回写：

```text
<codebook_root>evidence/<entry-id>/nodes.json
```

再读取 `<codebook_root>prompts/temp_typecard.json`，对 `container_summary` 与 `deep_dive` 节点按 `typecard-policy.json.batch_generation.batch_size` 批量调用大模型生成 TypeCard。`metadata_only` 是唯一允许规则确定性生成的 TypeCard 类型；除此之外，禁止用 Python、Node、正则、模板字符串或临时脚本替代大模型生成 TypeCard。若批量生成失败，按 `failure_policy=bisect_then_single` 先二分重试，再降级单节点生成；仍失败的节点写入 `document_errors` 和中文失败原因。`metadata_only` 节点不调用大模型，直接写入同一输出文件：

```text
<codebook_root>evidence/<entry-id>/typecards.json
```

`typecards.json` 必须只包含 `metadata_only`、`container_summary` 和 `deep_dive` 生成的 TypeCard；`evidence_only` 与 `skipped` 节点只能通过 `nodes.json` 和 `typecard-plan.json` 被后续文档追溯。
...
- `callers`
- `callees`
- `relation_skipped_reason`
- `source_nodes`

`behaviors` 中的每条行为也必须包含 `callers`、`callees` 和 `relation_skipped_reason`。非 `deep_dive` 行为的 callers/callees 固定为空数组；`deep_dive` 行为若查询成功但无结果，也必须保留空数组而不是省略字段。
...
通过门禁的入口独立使用 prompt 模板调用大模型生成**完整文档**（非局部片段）。每份文档必须使用该入口全部证据一次性生成，禁止关键 TypeCard 抽样、禁止局部片段输出、禁止以增量追加代替完整文档。禁止用 Python、Node 或临时脚本根据 TypeCard 拼接正式 Markdown 文档；脚本只能负责组装 prompt 输入、写临时文件、校验和原子重命名。每个入口需产出以下全部文档，文档列表和内容均完整：
...
- 文档生成记录包含 `generation_mode = "llm_prompted"`；若不是该模式，不得加入 `complete_documents`
...
3 轮后仍不达标则写入 `<codebook_root>proposals/iidp-codebase-scan-<date>.md`，状态为 `pending`。
...
```json
{
  ...
  "performance": {
    ...
    "callees_queries": 0,
    "typecard_batches": 0,
    "typecard_batch_retries": 0,
    "metadata_only_typecards": 0,
    "container_summary_typecards": 0,
    "deep_dive_typecards": 0
  },
  "total_modules": 0,
...
- 每个入口的符号枚举策略、枚举 kind 数、逐文件逐 kind 统计和 kind 查询失败数
- 每个入口的 TypeCard 计划动作分布：metadata_only、container_summary、deep_dive、evidence_only、skipped
- 每个入口是否启用关联扩展、基础文件数、扩展加入文件数和被拒绝候选数
- 每个入口的符号枚举和完整文档门禁状态
- CLI 命令失败数与节点数量对账失败数
- callers/callees 查询数、TypeCard 批次数、批量重试数和 metadata_only 生成数
- query 是否达到 limit
- UI 布局 JSON 是否生成
- 质量门禁是否通过
...
- [ ] codegraph 索引确认可用
- [ ] `quality.json.generation_mode == "llm_prompted"`，且本次不是 `rule_based_harness`
- [ ] 未使用 Python、Node 或临时脚本替代大模型生成 `container_summary`、`deep_dive` TypeCard、入口文档、系统文档或 SRS
...
- [ ] 每个入口生成 `typecard-plan.json`，且每个计划节点包含 `typecard_action`、`semantic_kind`、`action_reason` 和 `source_nodes` 追溯依据
- [ ] 每个入口逐文件、逐 `symbol_enumeration.kinds` 执行 `query_by_kind` 查询并与 `nodeCount` 完成数量对账
- [ ] Java 文件会分别查询 `class/method/field/parameter/import` 等完整枚举 kind，且不回退到单次 all-kind `path:<精确文件路径>` 查询
- [ ] 每个 kind 查询结果已按 `node.filePath` 与 `node.kind` 精确过滤，并按 `node.id` 去重合并
- [ ] `@Service` 被归类为 `service`，仅其 public 业务方法进入 `deep_dive`
- [ ] `@RestController` 或 `@Controller` 被归类为 `controller`，路由方法进入 `deep_dive`
- [ ] `@Entity`、`@Table` 或 `@TableName` 被归类为 `entity`，字段使用 `metadata_only`
- [ ] `@Autowired`、`@Inject` 或 `@Resource` 字段被归类为 `dependency`，不触发 callers/callees
- [ ] `field/property/enum/enum_member/type_alias/constant` 默认走 `metadata_only`，不调用大模型和 callers/callees
- [ ] 只有 `deep_dive` 节点执行 callers/callees；`container_summary` 与 `metadata_only` 不得因为缺少调用关系报错
...
- [ ] `quality.json` 写入 CLI 版本、已执行命令、查询失败、对账失败、`symbol_enumeration`、`entry_coverage`、`typecard_planning`、`performance`、配置快照、截断警告和 UI 状态
- [ ] 汇总报告展示给用户
